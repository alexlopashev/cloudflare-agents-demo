...
if not test -f mise.toml
    echo "Run activation from the repository root." >&2
    return 1
end
...
if not test -x $MISE_INSTALL_PATH
    echo "The repository-local toolchain is missing; source scripts/bootstrap.fish first." >&2
    return 1
end
